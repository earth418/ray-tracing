...
CUDA_HEAD RayIntersectInfo::RayIntersectInfo() {
    distance = INFINITY;
    hitObj = NULL;
...
TriMesh::TriMesh(const char* filename) {
    ...
    while (true) {
        ...
        if (strcmp("v", linehead) == 0) {
            Vec3 vert;
            int numRead = fscanf(objfile, "%f %f %f\n", &vert.x, &vert.y, &vert.z);
            if (numRead != 3) {
                fclose(objfile);
                std::cerr << "Error parsing Vertexes; sorry.\n";
                return;
            }
            ...
            Vec3 uv;
            int numRead = fscanf(objfile, "%f %f\n", &uv.x, &uv.y);
            if (numRead != 3) {
                fclose(objfile);
                std::cerr << "Error parsing UVs; sorry.\n";
                return;
            }
            uvs.push_back(uv);
        }
            ...
            if (numRead != 3) {
                fclose(objfile);
                std::cerr << "Error parsing Normals; sorry.\n";
                return;
            }
            ...
            int numRead = fscanf(objfile, "%d/%d/%d %d/%d/%d %d/%d/%d\n", 
...
CUDA_HEAD bool TriMesh::RayObjectIntersect(Vec3 rayOrigin, Vec3 rayDirection, RayIntersectInfo& info, 
...
CUDA_HEAD bool Sphere::RayObjectIntersect(Vec3 rayOrigin, Vec3 rayDirection, RayIntersectInfo& info, 
float minDist, float maxDist) const {
